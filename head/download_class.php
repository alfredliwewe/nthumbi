...
class PDF extends FPDF{
	...
	function print_line_data($number, $name, $regnumber, $points, $studentId)
	{
		...
		while ($row = $sql->fetchArray()) {
			$subId = $row['id'];
			$get_score_sql = $db->query("SELECT * FROM scores WHERE subject = '$subId' AND student = '$studentId' AND form = '$form' AND term = '$term' AND year = '$acayear' AND `group` = '$group' ");
	...
	function print_line_data_bottom($number, $name, $regnumber, $points, $studentId)
	{
		...
		while ($row = $sql->fetchArray()) {
			...
			if ($i == $subjects_count) {
				$this->Cell($cell_width,9,$score_mark,'LRTB','T','L',false);
			}
			else{
				$this->Cell($cell_width,9,$score_mark,'LTB','T','L',false);
			}
			$i += 1;
		}
	}
...
if (isset($_SESSION['form'], $_SESSION['term'])) {
	...
	if ($form < 3) {
		header("location: download_class_junior.php");
	}
	...
	$pdf->sub_heading('Form '.$form.' '.$group.' End of Term '.$term.' Student Ranking Report - '.acayear($acayear).' - Enrollment: '.$enrollment);
	$pdf->Ln();
	$pdf->Ln();
	$pdf->print_line();
	$pdf->Ln();
		...
		foreach ($mega as $key => $val) {
			...
			if ($total_count < 6) {
				...
				if ($calculated[3] == true) {
					$pdf->print_line_data_bottom($i, $student_name, $regnumber, $calculated[0], $studentId);
