...
class PDF extends FPDF{
	...
	function print_line_data($number, $name, $regnumber, $points, $studentId, $aggregate)
	{
	...
	function print_line_data_bottom($number, $name, $regnumber, $points, $studentId, $aggregate)
	{
		$this->Cell(10,9,$number,1,'T','L',false);
		$this->Cell(45,9,$name,1,'T','L',false);
		$this->Cell(21,9,$regnumber,1,'T','L',false);
...
if (isset($_SESSION['form'], $_SESSION['term'])) {
	...
	$pdf->sub_heading('Form '.$form.' '.$group.' End of Term '.$term.' Student Ranking Report - '.acayear($acayear).' - Enrollment: '.$reg_students);
	$pdf->Ln();
	$pdf->Ln();
	$pdf->print_line();
	$pdf->Ln();
	...
	if ($count_student > 0) {
		...
		foreach ($mega as $key => $val) {
			...
			if ($total_count < 6) {
				...
				if ($calculated[3] == true) {
					if ($i == $all_students) {
						$pdf->print_line_data_bottom($i, $student_name, $regnumber, $calculated[0], $studentId, $calculated);
					}
					else{
						$pdf->print_line_data($i, $student_name, $regnumber, $calculated[0], $studentId, $calculated);
					}
					$pdf->Ln();
					$i += 1;
				}
		...
		foreach ($zatsala as $key => $val) {
			...
			$regnumber = $students_reg[$key];
			$calculated = aggregate_points($studentId);
			$papersWritten = $calculated[1];

			if (count($papersWritten) > 5 && $val != 0) {

				$total_count = count($papersWritten);
